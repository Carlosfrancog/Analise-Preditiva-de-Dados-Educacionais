...
# Usar path relativo à pasta onde cads.py está
...
def adicionar_sala(nome, codigo):
    ...
    try:
        ...
        conn.close()
        raise ValueError(f"Sala com esse nome ou código já existe: {e}")
    conn.close()

def remover_sala(sala_id):
    """Remove uma sala do banco de dados. Verifica se não tem alunos."""
    conn = get_conn()
    # Verifica se existem alunos na sala
    alunos = conn.execute("SELECT COUNT(*) FROM alunos WHERE sala_id=?", (sala_id,)).fetchone()[0]
    if alunos > 0:
        conn.close()
        raise ValueError(f"Não é possível remover sala com {alunos} aluno(s). Remova os alunos primeiro.")
    ...
    try:
        ...
        conn.close()
        raise ValueError(f"Erro ao remover sala: {e}")
    conn.close()
...
# Garante que as tabelas existem quando o módulo é importado
try:
    _migrate_db()  # Cria/atualiza tabelas se necessário
except Exception as e:
    print(f"[Aviso] Erro ao migrar BD: {e}")
